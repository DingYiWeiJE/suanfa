...
for (let i = 1; i <= n; i ++) {
  mask[i] = (1 << (i - 1)).toString(2).padStart(10, "0")
}

console.log(mask.join(","))
// [0,0000000001,0000000010,0000000100,0000001000,0000010000,0000100000,0001000000,0010000000,0100000000,1000000000]
```

这样就可以得到所有的对应的位置位1的01数组压缩状态了
